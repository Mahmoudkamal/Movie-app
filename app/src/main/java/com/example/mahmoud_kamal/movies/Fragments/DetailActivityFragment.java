...
import com.example.mahmoud_kamal.movies.adapters.Review_Recycle_adapter;
import com.example.mahmoud_kamal.movies.adapters.Trailer_Recycle_adapter;
...
/**
 * A placeholder fragment containing a simple view.f
 */
public class DetailActivityFragment extends Fragment {
    ...
    MovieInfo data;
    ArrayList<MovieInfo> review_data, Trailer_data;
    ...
    public MovieInfo Getdata(JSONObject jsonObject) {

        MovieInfo mydata = null;
        ...
        if (jsonObject == null || jsonObject.length() == 0) {
            ...
            try {
                ...
                poster_path = jsonObject.getString(keys.poster_path);
                String backdrop_path = jsonObject.getString(keys.backdrop_path);
                String release_date = jsonObject.getString(keys.release_date);
                double vote_average = jsonObject.getDouble(keys.vote_average);
                String overview = jsonObject.getString(keys.overview);
                mydata = new MovieInfo(poster_path, overview, release_date, title, backdrop_path, vote_average);
    ...
    public ArrayList<MovieInfo> getDataReview(JSONObject jsonObject) {
        ...
        if (jsonObject == null || jsonObject.length() == 0) {
            ...
            try {
                ...
                for (int i = 0; i < results.length(); i++) {
                    ...
                    String author = row.getString(keys.author);
                    String content = row.getString(keys.content);
                    mydata.add(new MovieInfo(author, content));
    ...
    public ArrayList<MovieInfo> getDataTrailer(JSONObject jsonObject) {
        ...
        if (jsonObject == null || jsonObject.length() == 0) {
            ...
            try {
                ...
                for (int i = 0; i < results.length(); i++) {
                    JSONObject row = results.getJSONObject(i);
                    MovieInfo index = new MovieInfo();
                    String trailer_name = row.getString(keys.trailer_name);
                    String trailer_key = row.getString(keys.trailer_key);
